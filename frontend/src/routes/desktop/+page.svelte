<script lang="ts">
  ...
  const saveState = debounce(() => {
    ...
    const state = {
      ...
      pinnedTerminals: session.pinnedTerminals,
      windows: [],
    };
  ...
  function parseHTTPResponse(bytes: Uint8Array): {
    ...
    if (sepIdx === -1) return { status: 502, headers: {}, body: bytes };
    ...
    for (const line of lines.slice(1)) {
      ...
      if (idx > 0) {
        ...
        if (key !== 'transfer-encoding') headers[key] = val;
      }
    }
      ...
      if (frame.type === FrameType.SessionSync) {
        try {
          const state = JSON.parse(new TextDecoder().decode(frame.payload)) as {
            ...
            desktopState?: {
              wallpaper?: string;
              tabs?: Array<{ chanID: number; label: string }>;
              pinnedTerminals?: number[];
            };
          };
          ...
          if (state.desktopState) {
            ...
            if (state.desktopState.pinnedTerminals) {
              session.pinnedTerminals = state.desktopState.pinnedTerminals;
            }
          }
        } catch {
  ...
  function renameTerminal(chanID: number, label: string) {
    const ch = session.ptyChannels.find(c => c.chanID === chanID);
    if (ch) {
      ch.label = label;
      saveState();
    }
...
>
  ...
  <main class="flex-1 overflow-hidden relative">
    {#if session.client}
      <!-- Terminal app — always mounted -->
      <div
        class="absolute inset-0"
        style="visibility: {session.activeApp === 'terminal' ? 'visible' : 'hidden'}"
      >
        <TerminalApp
          ...
          pinnedChannels={session.pinnedTerminals}
          onNewTerminal={openNewTerminal}
          onSelectChannel={selectChannel}
          onCloseChannel={closeTerminal}
          onRenameChannel={renameTerminal}
          onPinChannel={(id) => { session.pinTerminal(id); saveState(); }}
          onUnpinChannel={(id) => { session.unpinTerminal(id); saveState(); }}
        />
      </div>

      <!-- File manager — always mounted -->
      <div
        class="absolute inset-0"
  ...
  {#if session.client}
    <Dock
      ...
      onSwitchApp={(app) => { session.activeApp = app; }}
    />
  {/if}
</div>
